...
public class SinistreServiceImpl implements SinistreService {
    ...
    @Override
    public Sinistre create(Sinistre s) {
        // Example check: positive cost
        if (s.getCout() == null || s.getCout() < 0) throw new IllegalArgumentException("cout must be >= 0");
        return dao.save(s);
    }
    ...
    public List<Sinistre> findByContratId(Long contratId) {
        // dao has optimized join by contrat id
        return dao.findByContratId(contratId);
    }
    ...
    public Double totalCoutByClient(Long clientId) {
        return findByClientId(clientId).stream()
                .mapToDouble(s -> s.getCout() == null ? 0.0 : s.getCout())
                .sum();
    }

    @Override
    public List<Sinistre> findBefore(LocalDateTime dateTime) {
        // demonstrate stream usage (could call dao.findBefore)
        return dao.findAll().stream()
                .filter(s -> s.getDateTime() != null && s.getDateTime().isBefore(dateTime))
    ...
    public List<Sinistre> findByCoutGreaterThan(Double montant) {
        return dao.findAll().stream()
                .filter(s -> s.getCout() != null && s.getCout() > montant)
                .collect(Collectors.toList());
    }
    ...
    public List<Sinistre> findSortedByCoutDesc() {
        return dao.findAll().stream()
                .sorted(Comparator.comparing(Sinistre::getCout, Comparator.nullsLast(Comparator.reverseOrder())))
                .collect(Collectors.toList());
    }
}
